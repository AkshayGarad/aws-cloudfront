...
## 18.	Is CloudFront multi region?
Yes, CloudFront is a multi-region content delivery network (CDN) service provided by Amazon Web Services (AWS). CloudFront has a global network of edge locations that are spread across the world in various regions, including North America, South America, Europe, Asia, Australia, and Africa. 
When a user requests content through CloudFront, the request is automatically routed to the nearest edge location that has the requested content available. This helps to ensure that content is delivered quickly and efficiently to end users, regardless of their location.
In addition to its global network of edge locations, CloudFront also supports the use of AWS Regions and Availability Zones. This allows users to configure CloudFront to cache and deliver content from their own AWS infrastructure, such as Amazon S3 buckets, Elastic Load Balancers, or EC2 instances, which can be located in specific AWS Regions and Availability Zones.
